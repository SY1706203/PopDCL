    ...
    def plot_embed(ax1, ax2, idx):
        ...
        for i, idx in enumerate([all_neg, all_pos]):
            ...
            for j in range(len(idx)):
                ax1.plot([0, g_item[j][0] / norm[j]], [0, g_item[j][1] / norm[j]], [0, g_item[j][2] / norm[j]],
                         color=cmaps[i], alpha=0.1)
        ...
        for i, idx in enumerate([all_neg, all_pos]):
            ...
            ax2.hist(cos, 50, range=[0, np.pi], color=cmaps[i], weights=np.zeros_like(cos) + 1. / cos.size,
...
# visualize the final embeddings in model using t-SNE and save the figure
# random select 1000 users and 1000 items
def visualize_embedding(model, random_state=0, n=0, epoch=0):
    # get embeddings
    if args.modeltype == 'INFONCE_batch':
        user_embeddings = model.embed_user.weight.data.cpu().numpy()
        item_embeddings = model.embed_item.weight.data.cpu().numpy()
    elif args.modeltype == 'Gauss' or args.modeltype == 'Adaptive_Gauss' or args.modeltype == 'MoG':
        user_embeddings = model.embed_user_mean.weight.data.cpu().numpy()
        item_embeddings = model.embed_item_mean.weight.data.cpu().numpy()
    if n != 0:  # random select n users and n items
        np.random.seed(random_state)
        user_embeddings = user_embeddings[np.random.choice(len(user_embeddings), n)]
        item_embeddings = item_embeddings[np.random.choice(len(item_embeddings), n)]

    # visualize by t-SNE
    tsne = TSNE(n_components=2, init='pca', random_state=0)
    user_embeddings = tsne.fit_transform(user_embeddings)
    item_embeddings = tsne.fit_transform(item_embeddings)
    # plot
    plt.figure(figsize=(10, 10))
    # set title
    plt.title('GAT', fontsize=20, fontweight='bold')
    # change the color of the points according to their labels
    plt.scatter(user_embeddings[:, 0], user_embeddings[:, 1], marker='o', s=1, alpha=0.3, c='r', label='user')
    plt.scatter(item_embeddings[:, 0], item_embeddings[:, 1], marker='o', s=1, alpha=0.3, c='b', label='item')
    # user color in legend
    red_patch = mpatches.Patch(color='red', label='user')
    # item color in legend
    blue_patch = mpatches.Patch(color='blue', label='item')
    # set legend
    plt.legend(handles=[red_patch, blue_patch], fontsize=20)
    path = './visualization/{}'.format(args.modeltype)
    ensureDir(path)
    plt.savefig(
        './visualization/{}/embedding_{}_{}_{}_{}.pdf'.format(args.modeltype, args.dataset, args.modeltype, epoch, n))
    plt.close()
