...
def merge_dicts(dict1, dict2):
    ...
    return [i[1] for i in sorted(merged_dict.items(), key=lambda x: x[0])]
...
def helper_load(filename):
    ...
    return user_dict_list, item_dict, item_dict_list
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            try:
                ...
                user = int(line[0])
                trainUser.extend([user])
                trainItem.extend([0])
                user_dict_list[user] = [0]
                # continue
            # user_dict_list[user] = items
...
class Data:
    ...
    def load_data(self):
        self.train_user_list, train_item, self.train_item_list, self.trainUser, self.trainItem = helper_load_train(
            self.train_file)
        self.valid_user_list, valid_item, self.valid_item_list = helper_load(self.valid_file)
        self.test_ood_user_list, self.test_ood_item_list, _ = helper_load(self.test_ood_file)
        self.test_id_user_list, self.test_id_item_list, _ = helper_load(self.test_id_file)
        self.pop_dict_list = []
        ...
        for i in range(Max):
            if i not in self.items:
                self.items.append(i)
        self.n_items = len(self.items)
        print("n_users:" + str(self.n_users), "n_items:" + str(self.n_items), max(self.users), max(self.items))
        ...
        pop_user_valid = {key: len(value) for key, value in self.valid_user_list.items()}
        pop_item_valid = {key: len(value) for key, value in self.valid_item_list.items()}

        self.pop_item_all = merge_dicts(pop_item, pop_item_valid)  # only when dataset is segmentd as train and valid
        # print(len(self.pop_item_all))
        # print(max(self.pop_item_all.keys()))
        self.pop_item = pop_item
        self.pop_user = pop_user
        print("interactions:", self.n_observations)
        # neg_lambda, tau+ / tau-
        self.lambda_u = {key: (np.sum([pop_item[v] for v in value]) - 1) / (self.n_observations - 1) /
                              (1 - (np.sum([pop_item[v] for v in value]) - 1) / (self.n_observations - 1)) for
                         key, value in self.train_user_list.items()}
        self.sigma_pop_i = {key: np.sum([pop_item[v] for v in value]) for key, value in self.train_user_list.items()}

        self.pop_i = {key: len(value) for key, value in self.train_item_list.items()}
        # print(type(self.train_item_list))
        # print(type(self.valid_item_list))
        # print(type(self.pop_i))
        # print(type(self.pop_item_all))
        self.item_prior = {key: len(value) / self.n_observations for key, value in self.train_item_list.items()}
        self.user_prior = {key: len(value) / self.n_observations for key, value in self.train_user_list.items()}
        ...
        if self.modeltype == 'CausE':
            self.train_data = TrainDataset_cause(self.modeltype, self.users, self.train_user_list, self.n_observations, \
                                                 self.n_interactions, self.pop_item, self.n_items, self.infonce,
                                                 self.neg_sample, self.items, self.sample_items)
        else:
            self.train_data = TrainDataset(self.modeltype, self.users, self.train_user_list, self.user_pop_idx,
                                           self.item_pop_idx, \
                                           self.neg_sample, self.n_observations, self.n_items, self.sample_items,
                                           self.weights, self.infonce, self.items, self.item_prior, self.user_prior,
                                           self.lambda_u,
                                           self.pop_i, self.sigma_pop_i, self.pop_user)
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, modeltype, users, train_user_list, user_pop_idx, item_pop_idx, neg_sample, \
                 n_observations, n_items, sample_items, weights, infonce, items, item_prior, user_prior, lambda_u,
                 pop_i, sigma_pop_i, pop_u):
    ...
    def __getitem__(self, index):
        ...
        item_prior = self.item_prior[pos_item]
        user_prior = self.user_prior[user]
        lambda_u = self.lambda_u[user]
        pop_i = self.pop_i[pos_item]
        sigma_pop_i = self.sigma_pop_i[user]
        pop_u = self.pop_u[user]

        if self.infonce == 1 and self.neg_sample == -1:

            return user, pos_item, user_pop, pos_item_pop, pos_weight, item_prior, user_prior, lambda_u, pop_i, sigma_pop_i, pop_u
            ...
            return user, pos_item, user_pop, pos_item_pop, pos_weight, item_prior, user_prior, lambda_u, pop_i, sigma_pop_i, pop_u, torch.tensor(
                neg_items).long(), neg_items_pop
            ...
            return user, pos_item, user_pop, pos_item_pop, pos_weight, item_prior, user_prior, lambda_u, pop_i, sigma_pop_i, pop_u, neg_item, neg_item_pop
...
class TrainDataset_cause(torch.utils.data.Dataset):
    ...
    def __init__(self, modeltype, users, train_user_list, n_observations, n_interactions, pop_item, n_items, infonce,
                 neg_sample, items, sample_items):
        self.modeltype = modeltype
        self.users = users
        self.train_user_list = train_user_list
        self.n_observations = n_observations
